...
import { updatePlatformSchema, IUpdatePlatformForm } from "@/src/zod/platform.validation";
...
export async function updatePlatformAction(
    id: string,
    payload: IUpdatePlatformForm
): Promise<{ success: true; data: IPlatform } | ApiErrorResponse> {
    const parsed = updatePlatformSchema.safeParse(payload);
    if (!parsed.success) {
        return {
            success: false,
    ...
    try {
        const response = await httpClient.patch<IPlatform>(
            `/platform/${id}`,
